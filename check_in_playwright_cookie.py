...
class V2FreeCookieSign:
    ...
    def __init__(self, cookie_str: str):
        ...
        # 解析 cookie 字符串为字典列表，供 playwright 使用
    ...
    def run(self):
        with sync_playwright() as p:
            ...
            try:
                ...
                # 检查是否真的处于登录状态（简单判断：是否存在“签到”按钮或用户信息）
                ...
                if not clicked:
                    ...
                    result_text = "未找到签到按钮，可能今日已签到或页面结构变更"
                else:
                    # 3. 等待签到结果（Turnstile 验证会自动处理）
                    # 等待 3 秒让验证流程和结果出现
